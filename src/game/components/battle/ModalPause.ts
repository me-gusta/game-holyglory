...
class CardToggle extends BaseNode {
    ...
    resize() {
        ...
        // this.button.position.y = this.bg.height / 2 - this.button.height / 2
    }
}

class ButtonSurrender extends BaseNode {
    bg = create_sprite('button_card_danger')
    lbl: Text
    constructor() {
        super()
        this.lbl = create_text({ text: 'OK', style: { fontSize: 94, fill: colors.dark } })
        this.addChild(this.bg)
        this.addChild(this.lbl)
...
class CardSurrender extends BaseNode {
    bg = create_sprite('card_danger')
    button = new ButtonSurrender()
    icon: Sprite
    lbl: Text

    constructor() {
        super()
        this.icon = create_sprite('icons/flag')
        this.lbl = create_text({ text: 'Surrender?', style: { fontSize: 84, fill: colors.dark } })
        this.lbl.anchor.x = 0
...
export default class ModalPause extends BaseNode {
    bg: TilingSprite
    ...
    card_surrender = new CardSurrender()
    ...
    constructor() {
        ...
        this.vrow.add(this.card_surrender)
        ...
        this.card_surrender.button.on('pointerup', () => {
            this.trigger('surrender')
        })
    }
